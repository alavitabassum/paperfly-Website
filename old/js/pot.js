        ...
        {
            //alert($('#retReason').val()+' || '+$('#orderid').val()+' || '+$('#retRemarks').val());
            var pickempid = 'na';
            ...
            $.ajax({
                ...
                success: function (response)
                {
                    if (response == 'success')
                    {
                        ...
                        $("#NoShow" + orderid).attr("disabled", true);
                        $("#Rea" + orderid).attr("disabled", true);
                        $("#DropDP2" + orderid).attr("disabled", false);
                        setTimeout($('#returnModal').css('display', 'none'), 2000);
                    } else
                    {
        ...
        function holdUpdate(ord)
        {
            if (document.getElementById("dropemp" + ord).value == document.getElementById("usercode").value)
            {
                $('#onHold' + ord).datetimepicker(
                {
                    dateFormat: 'dd-mm-yy',
                    onClose: function (selected)
                    {
                        var inpVal = $('#onHold' + ord).val();
                        if (inpVal != "")
                        {
                            //
                            var pickempid = ($("#dropemp" + ord).val());
                            ...
                            $.ajax({
                                ...
                                success: function (msg)
                                {
                                    if (msg == 'success')
                                    {
                                        $("#Rea" + ord).css("background-color", "yellow");
                                        $("#Rea" + ord).html($("#onHold" + ord).val().substr(0, 2));
                                        $("#Rea" + ord).attr("disabled", true);
                                        $("#altermsg").css("color", "red");
                                        $("#altermsg").text(ord + " Order on Hold");
                                    } else
                                    {
                                        $("#altermsg").css("color", "red");
                                        $("#altermsg").text("Unable to update status");
                        ...
                        $('#onHold' + ord).css('display', 'none');
                    }
                });
                $('#onHold' + ord).css('display', '');
                $('#onHold' + ord).focus();
